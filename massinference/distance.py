...
#        return (c/self.h)*integrate.romberg(f,z1,z2,(om,ol,ok))/1e5
    ...
    def h_squared(self, z):
        ...
        Hsq = (H0 ** 2.) * (self.OMEGA_M * (1. + z) ** 3. + (
        1. - self.OMEGA_M))  # Flat LambdaCDM only at this stage
        return Hsq

    def rho_crit_univ(self, z):  # critical density of the universe at z
        rho = (2.642 * 10 ** 46) * self.h_squared(
            z)  # units of solar mass per cubic megaparsec, H(z) must be in units of per second.
        return rho
